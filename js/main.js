...
const QUALITY_CATEGORIES = [
  ...
  },
  {
    key: 'minor',
    label: 'Minor',
    test: q => q.key === 'min' || q.key.startsWith('m')
  },
  {
    key: 'dominant',
    label: 'Dominant',
    test: q => /^7|^9|^11|^13/.test(q.key)
  },
  {
    key: 'sus',
    label: 'Sus',
    test: q => q.key.includes('sus')
  },
  {
    key: 'dim',
    label: 'Dim',
    test: q =>
      ...
      q.key.includes('m7b5') ||
      q.key.includes('m9b5') ||
      q.key.includes('m11b5')
  },
  {
    key: 'aug',
    label: 'Aug',
    test: q => q.key.includes('aug') || q.key.includes('#5')
  },
  {
    key: 'power',
    label: 'Power',
...
function qualityLabel(key) {
  return availableQualities.find(q => q.key === key)?.label ?? key;
}
  ...
  if (category.key === 'other') {
    ...
    QUALITY_CATEGORIES
      ...
      .forEach(c => {
        availableQualities
          .filter(c.test)
          .forEach(q => categorized.add(q.key));
      });
...
function init() {
  ...
  $('capoSelect').innerHTML = Array.from(
    { length: 8 },
  ...
  $('familySelect').innerHTML = FAMILIES
    .map(f => option(f, SHAPE_LABELS[f] ?? f))
    .join('');

  $('usageSelect').innerHTML = USAGES
    .map(u => option(u, USAGE_LABELS[u] ?? u))
    .join('');

  $('levelSelect').innerHTML = LEVELS
    .map(l => option(l, l === 'all' ? 'すべて' : `Level ${l}`))
    .join('');

  $('voiceSelect').innerHTML = VOICES
    .map(v => option(v, v === 'all' ? 'すべて' : v))
    .join('');

  if (availableQualities.some(q => q.key === 'maj7')) {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') closeQualityPalette();
  });

  const advancedToggle = $('advancedToggle');
  const advancedBody = $('advancedFilterBody');
  ...
  if (advancedToggle && advancedBody) {
    advancedToggle.addEventListener('click', () => {
      ...
      advancedToggle.textContent = open ? '− 詳細' : '＋ 詳細';
    });
  }
...
function renderQualityPalette() {
  ...
  $('qualityFrequent').innerHTML = frequentItems
    .map(q => qualityChip(q, current, true))
    .join('');

  $('qualityTabs').innerHTML = QUALITY_CATEGORIES
    .map(cat => `
      <button
        class="quality-tab ${selectedQualityCategory === cat.key ? 'active' : ''}"
        type="button"
  ...
  const list = qualitiesForCategory(selectedQualityCategory)
    .filter(q => !frequentSet.has(q.key) || selectedQualityCategory !== 'other');

  $('qualityList').innerHTML = list.length
    ? list.map(q => qualityChip(q, current, false)).join('')
    : '<p class="quality-empty">該当するコードタイプがありません。</p>';
...
function qualityChip(q, current, compact) {
  ...
  return `
    <button
      class="quality-chip ${compact ? 'frequent' : ''} ${q.key === current ? 'selected' : ''}"
      type="button"
      data-quality="${q.key}"
    >
      <span>${q.label}</span>
...
function render() {
  ...
  const soundingBassPc = $('bassSelect').value === 'none'
    ? null
    : NOTE_NAMES.indexOf($('bassSelect').value);

  const capoFret = Number($('capoSelect').value || 0);

  const formRootPc = mod12(soundingRootPc - capoFret);
  const formBassPc = soundingBassPc === null
    ? null
    : mod12(soundingBassPc - capoFret);
  ...
  $('currentChord').textContent = capoFret
  ...
  $('cards').innerHTML = rows.length
    ...
    : `<div class="empty">条件に合うフォームがありません。Difficulty や詳細フィルタを緩めてください。</div>`;
}
...
function cardHtml(item) {
  return `
    <article class="card">
      ...
      <div class="diagram">
        ${renderDiagram(item.frets, item.diagramRootPc ?? NOTE_NAMES.indexOf(item.root))}
      </div>
    </article>
  `;
}
